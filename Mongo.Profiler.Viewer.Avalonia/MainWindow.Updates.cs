...
{
    private const string DefaultLocalUpdateFeedPath = @"C:\mongo-profiler";
    ...
    private async Task CheckForViewerUpdateAvailabilityAsync(bool manualRequest)
    {
        ...
        try
        {
            var channel = ReadEnvironment("MONGO_PROFILER_VIEWER_UPDATE_CHANNEL");
            ...
            SetStatus(
                isPortable
                    ? "A newer viewer version is available. Portable mode can detect updates, but applying them may require installed mode."
                    : "A new viewer version is available. Click Update to apply.",
                StatusKind.Warning);
        }
        catch (Exception ex)
        {
    ...
    {
        var feedUrl = ReadEnvironment("MONGO_PROFILER_VIEWER_UPDATE_FEED_URL");
        ...
        var repoUrl = ReadEnvironment("MONGO_PROFILER_VIEWER_UPDATE_REPO_URL");
        if (!string.IsNullOrWhiteSpace(repoUrl))
        {
            var includePreRelease = ReadEnvironmentAsBool("MONGO_PROFILER_VIEWER_UPDATE_PRERELEASE", defaultValue: false);
            var token = ReadEnvironment("MONGO_PROFILER_VIEWER_UPDATE_GITHUB_TOKEN");
